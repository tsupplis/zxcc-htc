 ...
 *	CP/M-80 version
 *
 *	C [-Y] [-C] [-O] [-I] [-F] [-U] [-D] [-S] [-X] [-P] [-W] [-M] files {-Llib}
 */
...
main(int argc, char ** argv)
{
	...
	fprintf(stderr, "HI-TECH C COMPILER (CP/M-80) V3.09-P004\n");
	fprintf(stderr, "Copyright (C) 1984-2017 HI-TECH SOFTWARE\n");
#if	EDUC
	...
	while(*++argv) {
        ...
        if(overlay && reloc) {
            fprintf(stderr,"-Y and -A options are incompatible, disabling overlay\n");
            overlay=0;
        }
		nfiles++;
		cp = argv[0];
...
setup()
{
	...
	if(strcmp(fcbname(fileno(stdout)), "CON:") && 	
	   strcmp(fcbname(fileno(stdout)), "RSX:")) {	
